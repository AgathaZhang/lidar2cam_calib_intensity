...
public:
    ...
    RosNode(const string& configPath) {
        ...
        airylidar = nh.subscribe("/robosense/rs_96_airy_points", 10, &RosNode::ROBOSENSE_AIRY_96_handler, this);        /*速腾雷达适配*/
        ...
        "/lidar_topic", 10,
        &RosNode::ROBOSENSE_AIRY_96_handler, this);
    ...
    void ROBOSENSE_AIRY_96_handler(const sensor_msgs::PointCloud2::ConstPtr &msg){
        ...
        using PointType = pcl::PointXYZI;  // 定义点云类型，这里使用 PointXYZI 包含强度信息
        ...
        *pointcloud_surf_edge_full = *pointcloud_surf;// + *pointcloud_edge;
        // pcl::io::savePCDFileBinary("/home/kilox/workspace/lidar2cam_calib_ws/src/lidar2cam_calib/src/airy.pcd", *pointcloud_surf_edge_full);  // 以二进制格式保存点云到 PCD 文件
    ...
    // void syncCallback_airy(const sensor_msgs::PointCloud2::ConstPtr& laserMsg/*点云消息*/, const sensor_msgs::ImageConstPtr& imageMsg/*图像消息*/) {
...
private:
    ...
    ros::Subscriber airylidar;                   //!< airy信息订阅器
